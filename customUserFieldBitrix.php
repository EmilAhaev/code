...
Bitrix\Main\Loader::registerAutoLoadClasses(null, [
    'CUserTypeColor' => '/local/class/CUserTypeColor.php'
]);

use Bitrix\Main;
$eventManager = Main\EventManager::getInstance();
$eventManager->addEventHandler('main', 'OnUserTypeBuildList', ['CUserTypeColor', 'GetUserTypeDescription']);
...
{
    const USER_TYPE_ID = 'table_clear';
    ...
    function GetEditFormHTML($arUserField, $arHtmlControl)
    {
        if (!empty($arHtmlControl['VALUE'])) {
            $arHtmlControl['VALUE'] = json_decode(htmlspecialcharsBack($arHtmlControl['VALUE']), true);
        } else {
            $arHtmlControl['VALUE'] = [];
        }
        $html = '<table>';

        $iblockId = preg_replace("/[^0-9]/", '', $arUserField['ENTITY_ID']);
        $THIS_SECTION_ID = $arUserField['ENTITY_VALUE_ID'];

        $arFilter = ['ID' => $THIS_SECTION_ID];
        $arSelect = ['IBLOCK_SECTION_ID', 'NAME'];
        $db_list = \CIBlockSection::GetList([], $arFilter, false, $arSelect);
        if ($parentSection = $db_list->Fetch()) {
            //найдем примеси
            if ($parentSection['IBLOCK_SECTION_ID']) {
                $db_parent = \CIBlockSection::GetList([], ['ID' => $parentSection['IBLOCK_SECTION_ID'], 'IBLOCK_ID' => $iblockId], false, ['NAME' ,'UF_PRIMESI']);
                if ($arParens = $db_parent->Fetch()) {
                    if (!empty($arParens['UF_PRIMESI'])) {
                        foreach ($arParens['UF_PRIMESI'] as $key => $value) {
                            $val = '';
                            if (!empty($arHtmlControl['VALUE'][$key])) {
                                $val = $arHtmlControl['VALUE'][$key];
                            }

                            $html .= '<tr><td>'. $value .'</td><td> <input name="'. $arHtmlControl['NAME'] .'['. $key .']" value="'. $val .'" /></td></tr>';
                        }
                    }
                }
            }
    ...
    */
    ...
    function GetAdminListViewHTML($arUserField, $arHtmlControl)
    {
        if (strlen($arHtmlControl["VALUE"]) > 0) {
            ...
            $THIS_SECTION_ID = htmlspecialcharsbx($_REQUEST['SECTION_ID']);

            $db_parent = \CIBlockSection::GetList([], ['ID' => $THIS_SECTION_ID, 'IBLOCK_ID' => $iblockId], false, ['NAME', 'UF_PRIMESI']);
            if ($arParens = $db_parent->Fetch()) {
                if (!empty($arParens['UF_PRIMESI'])) {
                    foreach ($arParens['UF_PRIMESI'] as $key => $value) {
                        $val = '';
                        ...
                        $html .= '<tr><td style="border: 1px solid; padding: 2px 7px;">' . $value . '</td><td  style="border: 1px solid; padding: 2px 7px;">' .  $val . '</td></tr>';
                    }
                }
            }
            $html .= '</table>';
    ...
    */
    ...
    //custom
    public static function getFieldValue($arUserField, $arAdditionalParameters = array()) {
        return  json_decode(htmlspecialcharsBack($arUserField['VALUE']), true);
    }
